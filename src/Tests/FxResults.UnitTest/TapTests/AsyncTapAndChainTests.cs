...
[TestFixture]
public class AsyncTapAndChainTests
{
    // Helpers for test flows
    private Task<Result<int>> AsyncStart(int n) => Task.FromResult(Result.Success(n));
    private Task<Result<int>> AsyncFail(string message) => Task.FromResult(Result.Fail<int>(message));
    private Task<Result<int>> AddAsync(int n, int amount) => Task.FromResult(Result.Success(n + amount));
    private Task<Result<string>> ToStringAsync(int n) => Task.FromResult(Result.Success($"Num:{n}"));
    ...
    public async Task Async_FluentChain_TapSync_SideEffect()
    {
        ...
        Assert.That(final.IsSuccess, Is.True);
        Assert.That(final.Value, Is.EqualTo(8));
        Assert.That(Logs, Is.EqualTo(new List<string>
        {
            "Sync log: 7",
            "Logged: Async log: 8"
