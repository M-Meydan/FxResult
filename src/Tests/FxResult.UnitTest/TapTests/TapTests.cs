...
public class TapTests
{
    ...
    private Result<int> GetUserId(string username) =>
        username switch
        {
            "admin" => Result<int>.Success(1001),
            "bob" => Result<int>.Success(1002),
            _ => Result<int>.Fail("User not found")
        };

    private Result<string> GetUserName(int userId) =>
        userId switch
        {
            1001 => Result<string>.Success("Admin User"),
            1002 => Result<string>.Success("Bob Builder"),
            _ => Result<string>.Fail("Unknown ID")
        };

    private Result<bool> IsUserActive(int userId) =>
        userId == 1001 ? true :
        userId == 1002 ? false :
        Result<bool>.Fail("User not found");
    ...
    {
        var log = new List<string>();
        var capturedResult2 = Result<string>.Fail("error");
        ...
        var result = GetUserId("admin")
            ...
            .Then(name => name.StartsWith("A") ? Result<string>.Fail("Name blocked") : name.ToUpper())
    ...
    public void Tap_Nested_Steps_And_MultiError_Paths()
    {
        ...
        foreach (var user in userIds)
        {
            var result = GetUserId(user)
                ...
                .Then(active => active ? Result<string>.Success("Allowed") : Result<string>.Fail("Inactive"))
